...
class RoomTestCase(TestCase):
    def setUp(self):
        ...
        self.server = Server.objects.create(owner=self.owner, name="TEST SERVER")

        self.category_room = models.Room.objects.create(
            name="category", room_type=2, server=self.server, parent=None
        )

        self.text_room = models.Room.objects.create(
            name="text", room_type=1, server=self.server, parent=self.category_room
        )
    ...
    def test_set_parent_for_category(self):
        ...
        new_category = models.Room.objects.create(
            name="new category", room_type=2, server=self.server, parent=None
        )
    ...
    def test_set_text_channel_as_parent(self):
        ...
        other_text = models.Room.objects.create(
            name="other text", room_type=1, server=self.server, parent=None
        )
    ...
    def test_set_room_from_diferent_server_as_parent(self):
        ...
        other_server = Server.objects.create(owner=self.owner, name="OTHER SERVER")

        other_category = models.Room.objects.create(
            name="other category", room_type=2, server=other_server, parent=None
        )
