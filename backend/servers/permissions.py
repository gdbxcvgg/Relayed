    ...
    def has_object_permission(self, request, view, obj):
        if not hasattr(view, "perm_server_path"):
            raise AttributeError(
                f"You need to add perm_server_path if you want to use {self.__class__.__name__} permission"
            )
...
class IsServerMember(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        return ServerMember.objects.filter(server=server, user=request.user).exists()
