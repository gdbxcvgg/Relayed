...
const UserProfile = ({ userId }: { userId: string }) => {
    ...
    const openMessage = async () => {
        const res = await api.post<RoomType>("users/@me/channels", {
            recipients: [user?.id],
        });
        if (res.status !== 200) return;
        ...
        return (
            <div className="flex flex-col mb-10">
                <div className="w-160 h-24 sm:h-32 flex bg-[#363636] sm:rounded-t-xl pt-12 sm:pt-16 px-6">
                    <div className="bg-[#121212] rounded-full flex justify-center items-center size-24 sm:size-32"></div>
                </div>
                <div className="pt-15 sm:pt-19 px-6 flex flex-col gap-5">
                    <div className="flex flex-col sm:flex-row justify-between gap-5 w-full">
                        <div>
                            <div className="bg-[#121212] w-55 max-w-full shrink h-6 mb-0.75 rounded-md"></div>
                            <div className="bg-[#121212] w-40 max-w-full shrink h-4 rounded-md"></div>
                        </div>
                    </div>
                    <div>
                        <div className="bg-[#121212] w-50 max-w-full shrink h-4 mb-0.75 rounded-md"></div>
                        <div className="bg-[#121212] w-35 max-w-full shrink h-4 rounded-md"></div>
                    </div>
                </div>
            </div>
        );
